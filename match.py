...
os.makedirs("/Users/scalzo/Desktop/vessel-seg/data/DSA/train/images/matched")

img_dir = "/Users/scalzo/Desktop/vessel-seg/data/DSA/train/images"
img_filenames = [fname for fname in os.listdir(img_dir) if fname.endswith(".jpg")]
img_filenames = sorted(filenames)

target_dir = "/Users/scalzo/Desktop/vessel-seg/data/DSA/train/targets"
target_filenames = [fname for fname in os.listdir(img_dir) if fname.endswith(".jpg")]
target_filenames = sorted(filenames)

tn_all,fp_all,fn_all,tp_all = 0
for file_index in range(len(img_filenames)):
    image = np.array(Image.open(os.path.join(img_dir,img_filenames[file_index])))
    target = np.array(Image.open(os.path.join(img_dir,target_filenames[file_index])))
    image_out,tn,fp,fn,tp=matched_filter(img=image, target=target)
    tn_all  += tn
    fp_all  += fp
    fn_all  += fn
    tp_all  += tp
    output = Image.fromarray(image_out)
    output_dir = "/Users/scalzo/Desktop/vessel-seg/data/DSA/train/{}_matched.jpg".format(filenames[file_index][:-4])
    output.save(output_dir)

acc = float(tp_all + tn_all) / float(tp_all + tn_all + fp_all + fn_all)
specificity = float(tn_all) / float(tn_all + fp_all)
print("tn",type(tn),tn_all)
print("fp",type(fp),fp_all)
print("fn",type(fn),fn_all)
print("tp",type(tp),tp_all)
print("acc",type(acc),acc)
print("specificity",type(specificity),specificity)

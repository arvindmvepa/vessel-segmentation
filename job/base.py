        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch_i in range(self.n_epochs):
                ...
                for batch_i in range(dataset.num_batches_in_epoch()):
                    ...
                    # train on batch
                    cost, cost_unweighted, layer_outputs, debug1, _, cur_objective_fn, regularization, reguarizer, \
                    cur_learning_rate, cur_op_fn = sess.run(
                        [network.cost, network.cost_unweighted, network.layer_outputs, network.debug1,
                         network.train_op, network.cur_objective_fn, network.regularization, network.regularizer,
                         network.cur_learning_rate, network.cur_op_fn],
                    ...
                    print('{}/{}, epoch: {}, cost: {}, cost unweighted: {}, batch time: {}, positive_weight: {},'
                          ...
                          'op function'.format(batch_num, self.n_epochs * dataset.num_batches_in_epoch(), epoch_i, cost,
                                               cost_unweighted, end-start, pos_weight, cur_objective_fn, regularization,
                                               reguarizer, cur_learning_rate, cur_op_fn))

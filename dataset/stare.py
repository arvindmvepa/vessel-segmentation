...
class StareDataset(Dataset):
    ...
    def __init__(self, batch_size=1, WRK_DIR_PATH='./stare', TRAIN_SUBDIR="train", TEST_SUBDIR="test", sgd=False):
    ...
    def get_images_from_file(self, DIR_PATH):
        ...
        for image_file,mask_file,target_file in zip(image_files, mask_files, target_files):
            ...
            top_pad = int((StareNetwork.FIT_IMAGE_HEIGHT - StareNetwork.IMAGE_HEIGHT) / 2)
            bot_pad = (StareNetwork.FIT_IMAGE_HEIGHT - StareNetwork.IMAGE_HEIGHT) - top_pad
            left_pad = int((StareNetwork.FIT_IMAGE_WIDTH - StareNetwork.IMAGE_WIDTH) / 2)
            right_pad = (StareNetwork.FIT_IMAGE_WIDTH - StareNetwork.IMAGE_WIDTH) - left_pad
